  ...
  if (previewUrl) {
    return (
      <img
        src={previewUrl}
        alt="önizleme"
        className={`${dim} object-contain rounded-lg bg-gray-950`}
      />
    )
  }
  return (
    <div className={`${dim} flex items-center justify-center rounded-lg bg-gray-950`}>
      <FileIcon mime={mime} />
    </div>
  )
}
...
export default function FileTransfer({
  ...
  return (
    <div className="w-full bg-gray-900 border border-gray-800 rounded-2xl p-4 flex flex-col gap-4">
      ...
      {/* ── Resume talebi ── */}
      {resumeRequest && (
        <div className="bg-yellow-950 border border-yellow-800 rounded-xl p-3 flex flex-col gap-2">
          <p className="text-sm text-yellow-300 font-medium">Yarıda kalan transfer</p>
          <p className="text-xs text-gray-400">
            <span className="text-white">{resumeRequest.name}</span>
      ...
      >
        ...
        {sending ? (
          ...
          <span className="text-gray-400 text-sm">Karşı taraf inceliyor...</span>
        ) : dcReady ? (
          <>
      ...
      {/* ── Gönderen: alıcı kararını bekliyor ── */}
      {waitingAccept && (
        <div className="flex flex-col gap-2 bg-gray-800 rounded-xl p-3">
          <div className="flex items-center gap-3">
            <PreviewThumb previewUrl={waitingAccept.previewUrl} mime={null} size="sm" />
            <div className="flex flex-col min-w-0 flex-1">
              <span className="text-sm text-white truncate">{waitingAccept.name}</span>
              <span className="text-xs text-yellow-400">Karşı tarafın onayı bekleniyor…</span>
            </div>
          </div>
          <button
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between text-xs text-gray-400">
            ...
            <span className="shrink-0 text-right">
              {fmtBytes(sendSpeed)}/s{sendEta > 0 && ` — ${fmtEta(sendEta)} kaldı`}
            </span>
          </div>
          ...
          <div className="flex justify-between items-center">
            <span className="text-xs text-gray-500">{sendProgress.toFixed(1)}%</span>
            <button onClick={cancelTransfer} className="text-xs text-red-400 hover:text-red-300 transition-colors">
              İptal
            </button>
          </div>
        </div>
      ...
      {pendingFiles.length > 0 && (
        <div className="flex flex-col gap-2">
          <p className="text-xs text-gray-500 uppercase tracking-wider">Gelen Dosya</p>
          {pendingFiles.map(f => (
            <div key={f.id} className="bg-gray-800 rounded-xl overflow-hidden">
              {/* Önizleme alanı */}
              <div className="w-full bg-gray-950 flex items-center justify-center p-2 min-h-[80px]">
                {f.previewUrl ? (
                  <img
                    src={f.previewUrl}
                    alt="önizleme"
                  ...
                  <div className="flex flex-col items-center gap-1 py-3">
                    ...
                    <span className="text-xs text-gray-600">Önizleme yükleniyor…</span>
                  </div>
                )}
              </div>
              {/* Dosya bilgisi + butonlar */}
              <div className="p-3 flex items-center justify-between gap-3">
                ...
                <div className="flex gap-2 shrink-0">
                  <button
                    onClick={() => declineFile(f.id)}
                    className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white text-xs font-semibold rounded-lg transition-colors"
                  >
                    Reddet
                  </button>
                  <button
                    onClick={() => acceptFile(f.id)}
      ...
      {incomingMeta && (
        <div className="flex flex-col gap-1.5">
          ...
          <span className="text-xs text-gray-500">{recvProgress.toFixed(1)}%</span>
        </div>
      )}
