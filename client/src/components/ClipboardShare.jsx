...
export default function ClipboardShare({
  ...
  return (
    <div className="w-full bg-gray-900 border border-gray-800 rounded-2xl p-4 flex flex-col gap-3">
      <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider">Metin / Pano</p>
      ...
      <textarea
        ...
        rows={3}
        className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-emerald-500 disabled:opacity-40 resize-none"
      />
      ...
      <div className="flex gap-2">
        <button
          ...
          disabled={!dcReady}
          className="px-3 py-2 bg-gray-700 hover:bg-gray-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-semibold rounded-xl transition-colors"
        >
          Panodan Yapıştır
        </button>
        <button
      ...
      {received.length > 0 && (
        <div className="flex flex-col gap-2">
          <p className="text-xs text-gray-500 uppercase tracking-wider">Alınan</p>
          {received.map((r, i) => (
            <div key={i} className="bg-gray-800 rounded-xl p-3 flex flex-col gap-2">
              <p className="text-sm text-white whitespace-pre-wrap break-all">{r.text}</p>
